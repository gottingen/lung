...
func initConfigs() {
	...
	unmarshalReader(remote, l.kvstore)
}
...
func initConfig(typ, config string) {
	...
	if err := unmarshalReader(r, l.config); err != nil {
		panic(err)
	}
}
...
func TestFindsNestedKeys(t *testing.T) {
	...
	for key, expectedValue := range expected {
		...
		assert.Equal(t, expectedValue, l.Get(key))
	}
...
`
	...
	var items []item
	err := l.UnmarshalKey("parent", &items)
	if err != nil {
		t.Fatalf("unable to decode into struct, %v", err)
	}
...
func newLungWithConfigFile(t *testing.T) (*Lung, string, func()) {
...
func newLungWithSymlinkedConfigFile(t *testing.T) (*Lung, string, string, func()) {
	watchDir, err := ioutil.TempDir("", "")
	require.Nil(t, err)
...
func TestWatchFile(t *testing.T) {
	...
	t.Run("file content changed", func(t *testing.T) {
		...
		v, configFile, cleanup := newLungWithConfigFile(t)
		defer cleanup()
	...
	t.Run("link to real file changed (à la Kubernetes)", func(t *testing.T) {
		...
		v, watchDir, _, _ := newLungWithSymlinkedConfigFile(t)
		// defer cleanup()
		wg := sync.WaitGroup{}
		v.WatchConfig()
		v.OnConfigChange(func(in fsnotify.Event) {
